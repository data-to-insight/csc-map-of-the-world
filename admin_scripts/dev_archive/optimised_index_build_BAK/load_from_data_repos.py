...
# Process a single file from data_repos
def process_repo_file(path):
    try:
        ...
        # Derive name using parent folder as tool label
        try:
...
# Main loader function for data_repos
def load_from_data_repos():
    repos_dir = Path("data_repos")
    entries, texts = [], []

    for path in repos_dir.rglob("*"):
        if path.suffix.lower() in [".md", ".txt", ".py", ".js", ".html"]:
            record = process_repo_file(path)
            if record:
