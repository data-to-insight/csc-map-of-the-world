...
(function () {
  function init() {
    ...
    // immediate type colouring
    function applyTypeColors(cy) {
      const sty = [
        { selector: 'node', style: { 'background-color': '#b8b8b8' } },                // default
    ...
    .then(async ({ nodes, edges }) => {
      ...
      // apply type colours right away unless user asked for degree colouring
      if (COLOR_MODE !== 'degree') {
        applyTypeColors(cy);
      }

      // degree sizing and add deg to data so we can color by it if requested
      const sizeFor = id => {
      ...
      // if colour by degree was requested, apply it now
      if (COLOR_MODE === 'degree') {
