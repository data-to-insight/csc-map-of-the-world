...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Debounced layout so we don't thrash it during chunking
  const runLayout = debounce(() => {
    cy.layout({
      name: "cose",            // swap to "fcose" if you includ plugin
      animate: true,
      padding: 50,
      fit: false,
  ...
  fetch(graphDataURL).then(r=>{
    ...
    function ensureTypeLoaded(typeCls) {
      ...
      // Start with the nodes of this type that aren't already in the graph
      ...
      // Pull in incident edges and the opposite endpoints so edges render immediately
      const incidentEdges = [];
      ...
      // Add in chunks, then run a layout once at the end to avoid pile-up at (0,0)
      addInChunks([
    ...
    function applyFilter(){
      ...
      const countByClass = {}; // avoid global ':visible' scans
      let visibleCount = 0;
